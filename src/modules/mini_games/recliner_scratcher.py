...
def recliner_instructions(op_cat, op_cat_color):
    ...
    print(f"Press {CYAN}'Enter'{RESET} to {CYAN}scratch{RESET} and {MAGENTA}meow{RESET} to {MAGENTA}play it cool{RESET}. ")


def scratcher_dynamics(chosen_cat, op_cat, m_cat_color, op_cat_color):

    print(f"You have 20 seconds and {state.lives_left} lives. The countdown starts when you press {CYAN}Enter{RESET}. Good luck!")
    input()
    ...
    while active and state.lives_left > 0:
        ...
        if current_time - start_time >= TIME_LIMIT:
            print(f"{RESET}No time left!")
            active = False

        elif current_time >= next_interrupt:
            print(f"{MAGENTA}{ITALICA}Someone´s coming! Type 'meow' and press 'Enter' to play it cool{RESET}")
            ...
            print(f"{CYAN}{ITALICA}Scratch now! (Press Enter){RESET}")
            user_input = input()
            ...
            if user_input == "":
                ...
                state.lives_left -= 1
                print(f"{RED}Press blank 'Enter', -1 life. Lives remaining: {state.lives_left}")
    ...
    if state.lives_left > 0:
        ...
        print(f"{RESET}Total scratches: {scratch_count}, succesful distractions: {meow_count} and lives left: {state.lives_left}")
